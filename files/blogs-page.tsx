...
const CATEGORY_COLORS: Record<string, string> = {
  ...
  "Investment":     "bg-[#C9A96E]/10 text-[#C9A96E] border-[#C9A96E]/20",
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="max-w-2xl mb-14">
          <div className="flex items-center gap-3 mb-4">
            ...
            <span className="text-xs text-[#C9A96E] uppercase tracking-widest font-medium">Insights</span>
          </div>
        ...
        {/* Featured post */}
        <div className="bg-[#12202E] border border-white/[0.06] hover:border-[#C9A96E]/20 rounded-2xl p-8 mb-8 transition-all group cursor-pointer">
          <div className="flex items-start justify-between gap-6 flex-wrap">
            <div className="max-w-2xl">
              <div className="flex items-center gap-3 mb-4">
                ...
                <span className="text-xs text-[#3A5060]">March 2025 · 5 min read</span>
              </div>
              <h2 className="font-serif text-2xl font-medium text-white mb-3 group-hover:text-[#E2C99A] transition-colors">
                {PLACEHOLDER_POSTS[0].title}
              </h2>
              <p className="text-[#5A7080] leading-relaxed">{PLACEHOLDER_POSTS[0].excerpt}</p>
            </div>
            <div className="flex items-center gap-2 text-sm text-[#C9A96E] group-hover:text-[#E2C99A] transition-colors flex-shrink-0 mt-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {PLACEHOLDER_POSTS.slice(1).map((post) => (
            <div
              key={post.title}
              className="group bg-[#12202E] border border-white/[0.06] hover:border-[#C9A96E]/20 rounded-2xl p-6 transition-all cursor-pointer flex flex-col"
            >
              <div className="flex items-center justify-between mb-4">
                <span className={`text-xs px-2.5 py-1 rounded-full border font-medium ${CATEGORY_COLORS[post.category] ?? "bg-white/5 text-[#8A9BAE] border-white/10"}`}>
                  {post.category}
                </span>
                <span className="text-xs text-[#3A5060]">{post.readTime}</span>
              </div>
              <h3 className="font-serif text-lg font-medium text-white mb-3 group-hover:text-[#E2C99A] transition-colors line-clamp-2 flex-1">
                {post.title}
              </h3>
              <p className="text-sm text-[#5A7080] leading-relaxed line-clamp-3 mb-5">{post.excerpt}</p>
              <div className="flex items-center justify-between pt-4 border-t border-white/[0.04]">
                <span className="text-xs text-[#3A5060]">{post.date}</span>
                <span className="flex items-center gap-1.5 text-xs text-[#C9A96E] group-hover:text-[#E2C99A] transition-colors">
        ...
        {/* CMS notice */}
        <div className="mt-14 bg-[#C9A96E]/5 border border-[#C9A96E]/15 rounded-2xl p-6 flex items-start gap-4">
          <BookOpen className="w-5 h-5 text-[#C9A96E] flex-shrink-0 mt-0.5" />
          <div>
            <p className="text-sm font-medium text-white mb-1">Blog content is static for now</p>
            <p className="text-sm text-[#5A7080]">
              A full CMS integration (Contentlayer, Sanity, or a <code className="text-[#C9A96E] bg-white/[0.04] px-1.5 py-0.5 rounded">posts</code> MongoDB collection) can be added in a future phase to enable dynamic publishing directly from the admin dashboard.
            </p>
          </div>
        </div>
      </div>
